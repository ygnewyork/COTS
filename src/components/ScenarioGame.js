...
export default function ScenarioGame() {
    ...
    const [paymentHistory, setPaymentHistory] = useState(user?.paymentHistory || 50); // 0-100 scale
    const [utilization, setUtilization] = useState(user?.creditUtilization || 30); // percentage
    const [badges, setBadges] = useState([]);
    const [feedback, setFeedback] = useState(null);
    const [gameStatus, setGameStatus] = useState("playing");

    // Sync final score to backend when game finishes
    useEffect(() => {
        if (gameStatus === "finished" && user) {
            updateUser({
                creditScore: score,
    ...
    }, [gameStatus, score, paymentHistory, utilization, user, updateUser]);
    ...
    const handleChoice = (choice) => {
        ...
        setFeedback({
            ...
            badge: choice.badge,
            outcomeOverride: choice.outcomeOverride,
        });
                            ...
                            >
                                ...
                                <div className="flex items-end gap-4 mb-4">
                                    ...
                                    >
                                        <img
                                            src={
                                                feedback.outcomeOverride === "positive" || (feedback.scoreImpact > 0 && feedback.outcomeOverride !== "negative")
                                                    ? "/happy_mascot.png"
                                                    : feedback.outcomeOverride === "negative" || feedback.scoreImpact < 0
                                                    ? "/sad_mascot.png"
                                                    : "/mascot.png"
                                            }
                                            alt={
